...
TEST_F(GameTest, BlockPatternIsStable) {
    ...
    std::unordered_set<Point> block = {{0, 0}, {1, 0}, {0, 1}, {1, 1}};
    setState(block);
...
TEST_F(GameTest, BlinkerPatternOscillates) {
    ...
    std::unordered_set<Point> horizontalBlinker = {{0, 1}, {1, 1}, {2, 1}};
    setState(horizontalBlinker);

    // Generation 1 should turn to vertical
    nextGeneration();
    std::unordered_set<Point> verticalBlinker = {{1, 0}, {1, 1}, {1, 2}};
...
TEST_F(GameTest, GliderPatternMoves) {
    ...
    std::unordered_set<Point> glider_gen0 = {{1, 0}, {2, 1}, {0, 2}, {1, 2}, {2, 2}};
    setState(glider_gen0);

    // After 4 generations, it returns to the same shape, but shifted by (1, 1).
    std::unordered_set<Point> glider_gen4_expected = {{2, 1}, {3, 2}, {1, 3}, {2, 3}, {3, 3}};
